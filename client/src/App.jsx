...
const App = () => {
  ...
  // Determine if current route is an app route (dashboard, profile-setup)
  const isAppRoute = 
  ...
  return (
    <>
      ...
      <Routes>
        ...
        <Route 
          path="/dashboard" 
          element={isLoggedIn ? <PatientDashboard /> : <Navigate to="/auth" replace />}
        />
        
        {/* Profile Setup - Redirect to auth if not logged in */}
        <Route 
          path="/profile-setup" 
          element={isLoggedIn ? <ProfileCompletion /> : <Navigate to="/auth" replace />}
        />
